...
export default function TripMediaGallery({ tripId, media, onChange }) {
  ...
  async function handleFileChange(e) {
    ...
    try {
      for (const file of files) {
        await api.addTripMedia(tripId, file);
      }
      toast.success(files.length > 1 ? 'Media uploaded' : 'Media uploaded');
      onChange();
    } catch (err) {
  ...
  async function handleRemove(item) {
    ...
    try {
      await api.removeTripMedia(item.id);
